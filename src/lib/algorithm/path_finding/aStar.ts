...
import { initFuncCost, initiHeuristicCost } from "../../../utils/heuristics";
...
export const aStar = (grid: GridType, startTile: TileType, endTile: TileType) => {
    ...
    const heuristicCost = initiHeuristicCost(grid, endTile); // Initialize the heuristic cost for each tile
    const functionCost = initFuncCost(); // Initialize the function cost for each tile
    ...
    while (untraversedTiles.length > 0) { // Continue while there are untraversed tiles
        untraversedTiles.sort((a, b) => { // Sort the queue by function cost
            if (functionCost[a.row][b.col] === functionCost[b.row][b.col]) {
        ...
        if (currentTile) {  // If the current tile is valid
            if (currentTile.isWall) {
                continue; // Skip if the tile is a wall
            }
            if (currentTile.distance === Infinity) {
                break; // Break if the tile's distance is infinity
            }
            ...
            for (let i = 0; i < neighbors.length; i++) { // Iterate through each neighbor
                ...
                if (distanceToNeighbor < neighbors[i].distance) { // Check if a shorter path is found
                    ...
                    functionCost[neighbors[i].row][neighbors[i].col] = neighbors[i].distance + heuristicCost[neighbors[i].row][neighbors[i].col]; // Update the function cost for the neighbor
    ...
    while (current !== null) { // Backtrack until the start tile
